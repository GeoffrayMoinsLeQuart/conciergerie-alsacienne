    ...
    >
      ...
      <div className="flex h-[260px] flex-col justify-between p-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm text-gray-700 mt-auto">
          ...
          {modeGestion === 'Conciergerie' && revenuMensuel && <div>💰 {revenuMensuel} €</div>}
          {modeGestion === 'Gestion Locative' && loyer && <div>💶 {loyer} €</div>}
          {modeGestion === 'Conciergerie' && occupation && <div>📊 {occupation}%</div>}
        </div>
      </div>
    </Link>
  );
